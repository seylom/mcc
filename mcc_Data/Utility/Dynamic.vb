...
Public Module QueryExtensions
   ...
   '<Extension()> _
   'Public Function ContainsAny(ByVal value As String, ByVal keywords() As String) As System.Linq.Expressions.Expression(Of 
   '   Return keywords.Any(Function(s) value.Contains(s))
   'End Function

   '<Extension()> _
   'Public Function ContainsAll(ByVal value As String, ByVal keywords() As String) As Boolean
   '   Return keywords.All(Function(s) value.Contains(s))
   'End Function

   '<Extension()> _
   '  Public Function ContainsAny(Of T)(ByVal qry As IQueryable(Of T), ByVal Parameters As ICollection(Of T)) As Boolean
   '   For Each parameter In Parameters
   '      If qry.Contains(parameter) Then
   '         Return True
   '      End If
   '   Next
   '   Return False
   ...
   '<Extension()> _
   'Public Function ContainsAll(Of T)(ByVal qry As IQueryable(Of T), ByVal Parameters As ICollection(Of T)) As Boolean
   '   For Each parameter In Parameters
   '      If Not qry.Contains(parameter) Then
   '         Return False
   '      End If
   '   Next
   '   Return True
   ...
   <Extension()> _
   Public Function ContainsAny(ByVal str As String, ByVal values() As String, ByVal ignoreCase As Boolean) As Boolean
      If ignoreCase Then
         If Not (String.IsNullOrEmpty(str) Or values.Length = 0) Then
            For Each stval In values
               If str.ToLower.Contains(stval.ToLower) Then
                  Return True
               End If
            Next
         End If
   ...
   <Extension()> _
  Public Function ContainsAll(ByVal str As String, ByVal values() As String, ByVal ignoreCase As Boolean) As Boolean
      If ignoreCase Then
         If Not (String.IsNullOrEmpty(str) Or values.Length = 0) Then
            For Each stval In values
               If Not str.ToLower.Contains(stval.ToLower) Then
                  Return False
               End If
            Next
         End If
      Else
         If Not (String.IsNullOrEmpty(str) Or values.Length = 0) Then
            For Each stval In values
               If Not str.Contains(stval) Then
                  Return False
               End If
            Next
         End If
...
Public Module DynamicQueryable
   ...
   Public Function SortBy(Of T)(ByVal source As IQueryable(Of T), ByVal propertyName As String) As IQueryable(Of T)
      ...
      Dim descIndex As Integer = propertyName.IndexOf("DESC", StringComparison.OrdinalIgnoreCase)
      If descIndex >= 0 Then
