<%@ Page Title="" Language="VB" MasterPageFile="~/Areas/Admin/Views/Shared/Admin.Master" Inherits="System.Web.Mvc.ViewPage(of AdminUserQuestionCommentsViewModel)" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="mainSiteContent" runat="server">
   <div style="border-bottom: 1px solid #eaeaea; padding: 5px; margin-bottom: 10px;">
      <h1 class="admin-title">
         Question Comments</h1>
   </div>
   <% If Model.Comments Is Nothing Or Model.Comments.Count <= 0 Then%>
   <center>
      <b>- No comments for the specified question -</b>
   </center>
   <% Else%>
   ...
   <%=Html.AdvancedPager(Model.Comments, "QuestionCommentsByQuestion", "AskAdmin", New Integer() {10, 30, 50})%>
   <table class="edit-info datatable">
      ...
      <tbody>
         ...
         <tr id='<%="item_" & it.UserQuestionCommentID %>' class='<%= IIF(idx Mod 2 = 0,"even", "odd") %>'
            tag='<%= it.UserQuestionCommentID %>'>
            <td align="center">
               <input type="checkbox" tag='<%= it.UserQuestionCommentID %>' class='cb_ar' />
            </td>
            <td>
      ...
      $('#btnDelete,#btnDeleteTop').click(function() {
         if (confirm('Do you really want to delete the selected comments?\r\n This operation cannot be reverted')) {
            ...
            if (ar_list.length > 0) {
               ...
               if (ids.length > 0) {
                  var data = { 'Ids': ids };
                  Proxy.invoke('DeleteQuestionComments', data, function(result) {
